...
/// <summary>
/// Implémentation d'un service récupérant des données sur les auteurs.
...
public class AuteurService : IAuteurService
{
    ...
    /// <summary>
    /// Crée un record Auteur champs par champs depuis un reader obtenu d'une commande à la base de données
    /// </summary>
    ...
    public Auteur? GetAuteurByIdWithLivres(int id)
    {
        ...
        {
            command.CommandText = @"SELECT * FROM Auteur 
                                LEFT JOIN Livre_Auteur ON Auteur.id = Livre_Auteur.idAuteur 
                                LEFT JOIN Livre ON Livre_Auteur.ISSNLivre = Livre.ISSN 
                                WHERE Auteur.id = @id";
            command.Parameters.AddWithValue("id", id);
